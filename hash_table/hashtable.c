...
void hashtable_add(struct hashtable *this, char *item) {
    int idx = hash_string(item, this->size);
    if (hashtable_lookup(this, item) == 0) { // Item not found
        // Create new listset
        if (this->table[idx] == NULL) { // Do not override existing list
            struct listset *newListset = listset_new();
            this->table[idx] = newListset;
        }
    }
    listset_add(this->table[idx], item);
}
...
int hashtable_lookup(struct hashtable *this, char *item) {
    ...
    if (this->table[idx] != NULL) {
        return listset_lookup(this->table[idx], item);
    }
    return 0;
}
...
void hashtable_remove(struct hashtable *this, char *item) {
    int idx = hash_string(item, this->size);
    listset_remove(this->table[idx], item);
}
